...
$H_0: \mu = 60$   
$H_a: \mu \neq 60$   
With $\alpha = 0.05$ and $P = 0.953$, we have a $\texttt{p-value}$ larger than $\alpha = 0.05$, we cannot draw conclusion from this result.
...
coeff <- test$Estimate[2]
pval <- test$Pr...t..[2]
```
The coefficient of $\texttt{gdpPercap}$ of the regression model is `r coeff`. This tells us that there is a positive but not strong relation between $\texttt{gdpPercap}$ and $\texttt{lifeExp}$.  
The $\texttt{p-value}$ for the coefficient of $\texttt{gdpPercap}$ is `r pval`, which is less than $\alpha = 0.05$, telling us that we can reject the null hypothesis "cofficient of $\texttt{gdpPercap} = 0$" in favor of the alternative hupothesis: $\texttt{gdpPercap}$ is statistically signifigent for interpreting $\texttt{lifeExp}$.  
Graph 1 is the plot of Actual value vs. Fitted value, and has a which tells us that the model works fine.
...
for (i in 1:10) {
  ...
  kable(my_err)
```
Based on the training error, I will choose the model with \code{k_nn} $= 1$ because it has no error. However, if I look at CV misclassification rate, I will choose the model with \code{k_nn} $= 7$ because the CV misclassification rate steadly decreases when \code{k_nn} goes from $1$ to $7$ and starts to fluctuates after $7$.  
In practice, I will choose \code{k_nn} $= 7$. From the lecture, we know that \code{k_nn} $= 1$ always gives the smallest error on training datasets, which is the result from overfitting, and the training error steadly increased as we increasing the value of \code{k_nn}. The test error, on the other hand, has a U-shape as we increasing the value of \code{k_nn}. By using cross-validation, we are able to test our model on all data, and thus identify the optimal \code{k_nn} value which minimize the test error. Therefore, in this case, the model with \code{k_nn} $= 7$ avoids overfitting as well as gives a good prediction.  
